...
- **笔耕批注与按批注重写** — 行内/整章批注（问题/修改建议/备注），AI 先出修改计划再重写整章；新稿生成新版本，旧版进版本历史可回滚
- **写作经验沉淀** — 重写时把已采纳批注归类为项目写作经验库（同类去重累计），注入后续章节审查 prompt，越写越少犯同类错误
- **反 AI 味检测** — 规则化标准（套话/弱化副词/书面连接词/意义膨胀/情绪直说/排比三连）0-100 分，文风工坊真实检测，审查阶段按同一词表扣分
