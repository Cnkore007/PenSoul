...
## 2026-08-02：简介修复 + 墨韵合并与反 AI 规则配置化

- **简介**：`NovelOntology` 加独立 `description` 字段（旧数据曾复用 core_concept.inspiration，列表读取兼容回退）；`update_project` 写入独立字段，`list_project_metas` 返回真实简介——之前列表/概览永远显示空简介。
- **墨韵合并**：审校（一致性检查）并入墨韵页，侧边栏删除「审校」入口；墨韵页含文风指标、反 AI 检测、一致性审校、反 AI 味规则配置四块。
- **反 AI 规则配置化**：新增 `anti_ai.rs`（词表/计分/提示词全局配置，存 `_config/anti-ai-rules.json`）；检测器 `ai_flavor.rs` 改为配置驱动；写作/审查/批注重写 prompt 从配置注入「语言铁律」（原硬编码 ANTI_AI_RULES 迁移为默认值）；墨韵页规则编辑器可增删词表、改计分、改提示词，保存后立即生效。
- **状态**：已完成并提交（2 个提交）。cargo test 全绿，tsc 通过，clippy 无新增警告。
- **下次待办**：用户验收；反 AI 规则可加「新增分类」能力（当前编辑现有五类）。
