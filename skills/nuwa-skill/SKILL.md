---
name: nuwa-skill
description: Use when the user wants to distill a person, persona, or topic into a reusable Codex skill, update an existing perspective skill with new material, or diagnose a vague need such as better decisions, writing, product judgment, or risk analysis into the right thinker/framework skill.
---
...
本文档统一使用以下约定，避免把 Claude Code 的平台假设带进 Codex：

- `<skills_root>`：Codex 技能根目录。默认优先用 `$CODEX_HOME/skills`（通常是 `~/.codex/skills`）；如果你的 Codex 通过 `~/.agents/skills/` 做原生发现，则把 skill 创建到该目录或建立符号链接。
- `spawn_agent`：Codex 的子代理能力。能并行时就按 6 个研究维度并行；如果当前会话没有多代理能力，就按同样拆分顺序串行执行。
- `web` / 联网检索：指当前会话可用的联网搜索与页面读取能力。若网络受限，优先用户提供的本地资料，并在最终 Skill 中明确记录信息缺口。
- `update_plan`：用于跟踪阶段和检查点。是否展示给用户不重要，流程本身不能跳。
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查 `<skills_root>`，必要时补查 `~/.agents/skills/`）
...
**来源A：本地已有Skill**
扫描 `<skills_root>/*-perspective/` 与 `<skills_root>/*-framework/`，必要时补查 `~/.agents/skills/`，读取每个 `SKILL.md` 的 description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空，跳过此步，只从来源B推荐。
...
```
<skills_root>/[person-name]-perspective/
或
<skills_root>/[topic]-framework/
├── SKILL.md                          # 最终产物
...
**关键规则**：
- 每个 agent 必须把调研结果写入对应的 md 文件。不存文件的调研等于没做。
- **所有调研文件必须存在 skill 目录内部**（`references/research/`），绝对不要存到 `07-调研与分析/` 或其他外部目录。Skill 必须是自包含的，复制整个 skill 目录就能独立使用，不依赖任何外部文件。这是为开源分发设计的核心原则。
...
优先启动6个并行 agent，每个负责不同信息维度。Codex 中用 `spawn_agent`；如果当前会话不支持多代理，就按同样的 6 个维度顺序串行执行。
...
调用 `spawn_agent` 时，用以下结构给任务（以Agent 1著作为例）：
...
输出要求：
- 写入 <skill_dir>/references/research/01-writings.md
- 每条信息标注来源URL和可信度
...
- 视频字幕获取（已提供脚本，直接调用）：
  - **Step 1 下载字幕**：`bash <skill_dir>/scripts/download_subtitles.sh <YouTube_URL> [输出目录]`
    - 自动优先人工字幕 → 中文 → 英文 → 自动生成字幕
    - 输出SRT/VTT文件到指定目录
  - **Step 2 清洗为纯文本**：`python3 <skill_dir>/scripts/srt_to_transcript.py <input.srt> [output.txt]`
    - 去时间戳、序号、HTML标签、连续重复行
    - 输出干净的可阅读transcript → 存入 `sources/transcripts/`
  - 用户提供本地视频文件（无字幕）：如已安装视频转写类 skill，优先调用；否则使用当前会话可用的转写方式
- 播客：搜索transcript网站（podcastnotes.org等）
- 调研摘要生成（Phase 1.5 用）：`python3 <skill_dir>/scripts/merge_research.py <skill_dir>`
  - 自动扫描 `references/research/01-06.md`，统计来源数、一手/二手占比、关键发现
  - 输出Phase 1.5检查点的markdown表格，无需手动统计
- 质量自检（Phase 4 用）：`python3 <skill_dir>/scripts/quality_check.py <SKILL.md 路径>`
  - 自动检查6项通过标准：心智模型数量、局限性、表达DNA、诚实边界、内在张力、一手来源占比
  - 输出逐项PASS/FAIL和总结

#### 利用已安装的信息获取Skill

Phase 1启动前，**主动扫描 `<skills_root>`，必要时补查 `~/.agents/skills/`**，检查是否有可用于信息获取的 skill。如果有，在调研中优先调用，比盲搜更稳定高效：
...
**执行方式**：在调用 `spawn_agent` 时，把可用 skill 的名称和用途告知 agent，让 agent 在调研中按需调用。这比让 agent 自己盲搜效率高得多。
...
| 来源与署名 | 固定内容：`> 基于 [女娲 · Skill造人术](https://github.com/alchaincyf/nuwa-skill) 的方法论生成` + `> 原始项目作者：[花叔](https://x.com/AlchainHust)` + `> Codex 适配与维护：[yuc](https://github.com/Cyxxxxx)` + `> 当前版本：Codex 适配版` |
...
**⚠️ 必须使用当前会话可用的联网工具（如 Codex 的 `web` 能力）获取真实信息，不可跳过。**
...
#### Step 4: 输出
将完成的SKILL.md写入 `<skills_root>/[person-name]-perspective/SKILL.md` 或 `<skills_root>/[topic]-framework/SKILL.md`。

---

### Phase 4: 质量验证

生成Skill后，优先用独立 agent 执行3项测试（避免自评偏差）。如果当前会话没有多代理能力，就由主 agent 按同样清单顺序自测，并明确标注这是降级验证：

#### 4.1 已知测试（Sanity Check）
选3个此人公开表态过的问题，**spawn 一个独立 agent 并让其加载新 Skill 后回答**，对比实际立场。
- 方向一致 → 模型有效
- 偏离 → 回溯调整心智模型权重
...
Phase 4 验证通过后，优先启动双Agent精炼，进一步提升Skill可操作性。如果没有多代理能力，则按相同检查维度做两轮独立自审：
